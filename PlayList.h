// Title: PlayList.h
// Author: Josh Winton
// Purpopse: Function prototypes for playlist class
// Created: 10/17/18
...
public:
  ...
  PlayList(const PlayList &a_play_list)
      : LinkedSet(a_play_list) { // copy constructor
  ...
  bool remove(const Song &a_song)
      override;           // overridden remove preserves order of songs
...
private:
  ...
  //
  Node<Song> *getPointerTo(const Song &target, Node<Song> *&previous_ptr)
      const; // gets pointer to a node; used by remove
};
